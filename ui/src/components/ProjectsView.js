...
// A dedicated, reusable component for the nested breakdown view
  ...
  useEffect(() => {
    const fetchMeta = async () => {
        try {
            ...
            if (response.ok) setProjectMeta(await response.json());
        } catch (err) { console.error("Failed to fetch project metadata", err); }
    };
    if (yearlyData.length > 0) fetchMeta();
  }, [yearlyData, token]);
            ...
            <div className="border rounded-lg p-2 mb-4">
                ...
                <div className="max-h-48 overflow-y-auto">
                    {exportableItems.map(item => (
                        <label key={item} className="flex items-center p-2 hover:bg-gray-100 rounded-md">
