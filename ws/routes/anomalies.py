...
def check_for_anomalies(year, month, platform):
    ...
    """
    # Get all projects with costs in the specified month/year
    latest_costs = db.session.query(
        Billing.project_id,
        db.func.sum(Billing.cost).label('total_cost')
    ).filter(Billing.billing_year == year, Billing.billing_month == month, Billing.platform == platform)\
    ...
    for project_cost in latest_costs:
        ...
        # Get historical data (previous 6 months) for this project
        history = []
        for i in range(1, 7):
        ...
        # Define anomaly threshold (e.g., 2 standard deviations above average)
        threshold = average + (2 * std_dev)

        # To avoid flagging small, insignificant spikes, also require a minimum cost increase
        min_increase = 50 # e.g., must be at least $50 over the average
...
def get_unread_anomalies(current_user):
    anomalies = Anomaly.query.filter_by(is_acknowledged=False).order_by(db.desc(Anomaly.timestamp)).all()
    output = []
