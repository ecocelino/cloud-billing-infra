                ...
                <table className="min-w-full">
                    <thead>
                        <tr>
                            ...
                            {[...Array(13)].map((_, i) => ( // 12 months + 1 summary column
...
const GcpBudgetsView = () => {
    ...
    const fetchBudgets = useCallback(async () => {
        if (!token) return;
        try {
            const response = await fetch(`/api/budgets/${selectedYear}`, {
    ...
    const budgetTableData = useMemo(() => {
        return yearlyBillingData.map(project => {
            ...
            // 🔹 ADDED: Calculate yearly totals for the new summary column
            const yearlyBudget = monthlyDetails.reduce((sum, month) => sum + month.budget, 0);
            const yearlyActual = project.total_cost || monthlyDetails.reduce((sum, month) => sum + month.actual, 0);
            const yearlyRemaining = yearlyBudget - yearlyActual;

            return { ...project, monthlyDetails, yearlyBudget, yearlyActual, yearlyRemaining };
        });
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                {/* 🔹 ADDED: Header for the new Yearly Summary column */}
                                <th className="px-4 py-3 text-center font-medium text-gray-500 dark:text-gray-300 uppercase sticky right-0 bg-gray-50 dark:bg-gray-700/50 z-10 w-48">
                                    YEARLY SUMMARY
                                </th>
                            </tr>
                        </thead>
                            ...
                            {currentData.length > 0 ? (
                                currentData.map(project => (
                                    <tr key={project.project_id}>
                                        ...
                                        {/* 🔹 ADDED: Cell for the Yearly Summary data */}
                                        <td className="px-4 py-4 align-top sticky right-0 bg-white dark:bg-gray-800">
                                            <div className="flex flex-col gap-2 text-gray-800 dark:text-gray-200">
                                                <BudgetBar budget={project.yearlyBudget} actual={project.yearlyActual} />
                                                <div className="flex justify-between items-center mt-1">
                                                    <div className={`text-xs font-bold ${project.yearlyRemaining < 0 ? 'text-red-500 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                                                        <span>{project.yearlyRemaining < 0 ? "Over:" : "Left:"}</span>
                                                        <span className="ml-1">{formatCurrency(Math.abs(project.yearlyRemaining))}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                ...
                                <tr>
                                    <td colSpan={months.length + 2} className="text-center py-6 text-gray-400 dark:text-gray-500">
                                        No projects found
                                    </td>
                                </tr>
                            )}
